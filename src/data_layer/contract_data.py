...
class ContractData():
    def list_all_contracts(self):
        ...
        with open("data_layer/data_files/contracts.csv",encoding="utf-8") as file_stream:
            ...
            for row in dest_reader:
                info = Contract(row["contract_id"], row["customer_ssn"], row["employee_ssn"], row["vehicle_id"], row["loan_date"], row["end_date"], row["base_price"],
    ...
    def new_contract(self, contr):
        ...
        contr_list = [contr.contract_id,contr.customer_ssn,contr.employee_ssn,contr.vehicle_id,contr.loan_date,contr.end_date,contr.base_price,contr.return_date,contr.extensions,contr.total,contr.status]
